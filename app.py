...
def load_cost_database():
    ...
    return {
        'H': 0.01, 'Li': 0.5, 'Be': 2.0, 'B': 0.2, 'C': 0.01, 'N': 0.01, 'O': 0.0,
        'Na': 0.1, 'Mg': 0.05, 'Al': 0.02, 'Si': 0.05, 'P': 0.1, 'S': 0.01, 'Cl': 0.01,
        'K': 0.1, 'Ca': 0.05, 'Sc': 5.0, 'Ti': 0.2, 'V': 1.0, 'Cr': 0.1, 'Mn': 0.1,
        'Fe': 0.02, 'Co': 0.5, 'Ni': 0.1, 'Cu': 0.05, 'Zn': 0.05, 'Ga': 1.0, 'Ge': 2.0,
        'As': 0.5, 'Se': 0.5, 'Br': 0.1, 'Kr': 1.0, 'Rb': 1.0, 'Sr': 0.5, 'Y': 2.0,
        'Zr': 0.5, 'Nb': 1.0, 'Mo': 0.5, 'Tc': 50.0, 'Ru': 20.0, 'Rh': 100.0, 'Pd': 50.0,
        'Ag': 50.0, 'Cd': 0.5, 'In': 2.0, 'Sn': 0.2, 'Sb': 0.5, 'Te': 0.5, 'I': 0.2,
        'Xe': 10.0, 'Cs': 2.0, 'Ba': 0.5, 'La': 2.0, 'Ce': 2.0, 'Pr': 5.0, 'Nd': 5.0,
        'Pm': 1000.0, 'Sm': 5.0, 'Eu': 10.0, 'Gd': 5.0, 'Tb': 50.0, 'Dy': 20.0, 'Ho': 20.0,
        'Er': 20.0, 'Tm': 50.0, 'Yb': 10.0, 'Lu': 50.0, 'Hf': 2.0, 'Ta': 5.0, 'W': 1.0,
        'Re': 10.0, 'Os': 50.0, 'Ir': 100.0, 'Pt': 100.0, 'Au': 80.0, 'Hg': 0.5,
        'Tl': 5.0, 'Pb': 0.2, 'Bi': 2.0, 'Th': 50.0, 'U': 10.0
    }
...
def load_machinability_data():
    ...
    return {
        'Al': 200, 'Cu': 80, 'Mg': 300, 'Zn': 100, 'Pb': 150,
        'Fe': 50, 'Ni': 30, 'Ti': 20, 'Co': 15, 'Cr': 40,
        'W': 10, 'Mo': 15, 'Ta': 8, 'Nb': 20, 'Zr': 25,
        'Si': 10, 'C': 5, 'B': 5, 'N': 0, 'O': 0
    }
...
class IntegratedMaterialValidator:
        ...
        try:
            with NewMPRester(self.mp_api_key) as mpr:
                docs = mpr.materials.summary.search(
                    formula=formula,
                    fields=["material_id", "energy_above_hull", "formation_energy_per_atom", 
                           "is_stable", "theoretical", "symmetry"]
                )
                
                if not docs:
                    return {'found': False, 'score': 50, 'ehull': None, 'note': 'No en MP'}
                ...
                return {
                    ...
                    'formation_energy': round(best.formation_energy_per_atom, 4) if best.formation_energy_per_atom else None,
                    'crystal_system': best.symmetry.crystal_system if best.symmetry else 'Unknown',
    ...
    def cost_analysis(self, formula):
        ...
        try:
            ...
            for el, amt in comp.items():
                symbol = str(el)
                cost_per_g = self.cost_db.get(symbol, 10.0)  # Default $10/g si no existe
            ...
            return {
                ...
                'score': score,
                'breakdown': breakdown,
                'economic_viability': 'Excelente' if score == 100 else 'Buena' if score == 75 else 'Limitada' if score == 50 else 'No viable'
            }
        except Exception as e:
            return {'error': str(e)}
    ...
    def synthesizability_score(self, chemical, thermo, mechanical):
        ...
        kinetic_score = max(0, 100 - (n_elem - 2) * 15)  # Más elementos = más difícil cinéticamente
    ...
    def full_validation(self, formula):
        ...
        final_score = (
            results['synthesizability']['total_score'] * final_weights['synthesizability'] +
            results['economic']['score'] * final_weights['economic'] +
            results['mechanical'].get('machinability_index', 0) * final_weights['mechanical'] +
...
def create_radar_chart(results):
    ...
    categories = ['Sintetizabilidad<br>Química', 'Estabilidad<br>Termodinámica', 
                  'Maquinabilidad<br>CNC', 'Viabilidad<br>Económica', 'Factibilidad<br>Cinética']
...
def main():
    # Header
    st.markdown('<div class="main-header">🔬 GNoME Material Validator</div>', unsafe_allow_html=True)
    st.markdown('<div class="sub-header">Plataforma Integrada de Validación para Materiales Descubiertos por IA</div>', unsafe_allow_html=True)
    ...
    *Herramientas Integradas:*
    ...
    # Main tabs
    tab1, tab2, tab3 = st.tabs(["🧪 Validación Individual", "📊 Análisis de Lista", "📚 Base de Datos"])
    ...
    with tab1:
        ...
        with col1:
            formula = st.text_input("Fórmula Química", "LiFePO4", 
                                  help="Ejemplos: LiFePO4, YBa2Cu3O7, TiO2")
        
        with col2:
            if st.button("🚀 VALIDAR AHORA", use_container_width=True):
                with st.spinner("Analizando en múltiples dimensiones..."):
                    validator = IntegratedMaterialValidator(mp_api_key=mp_key if mp_key else None)
        ...
        if 'last_result' in st.session_state:
            ...
            with m1:
                st.metric("Sintetizabilidad", f"{results['synthesizability']['total_score']:.0f}%")
            with m2:
                st.metric("Estabilidad", f"{results['thermodynamic']['score']:.0f}%")
            with m3:
                st.metric("Maquinabilidad", f"{results['mechanical']['machinability_index']:.0f}%")
            with m4:
            ...
            with col_graph2:
                fig_gauge = create_gauge_chart(results['final_score'], "Score de Validación Total")
            ...
            with col_exp1:
                st.markdown(get_download_link(results, f"validation_{formula}.json", 
            ...
            with col_exp2:
                ...
                st.markdown(f'<a href="data:file/csv;base64,{b64}" download="summary_{formula}.csv">📥 Descargar CSV Resumen</a>', 
                          unsafe_allow_html=True)
    
    with tab2:
        st.subheader("Análisis Batch de Múltiples Materiales")
        st.info("Ingrese una lista de fórmulas (una por línea) para análisis comparativo")
        
        batch_input = st.text_area("Fórmulas (ej: LiFePO4, TiO2, YBa2Cu3O7)", 
                                  height=150,
                                  placeholder="LiFePO4\nTiO2\nSiO2\n...")
            ...
            if formulas:
                ...
                for i, formula in enumerate(formulas):
                    ...
                    results_list.append({
                        ...
                        'Sintetizabilidad': result['synthesizability']['total_score'],
                ...
                # Gráfico comparativo
                fig_comp = px.bar(df_batch, x='Formula', y='Score', 
                                 color='Status',
                                 color_discrete_map={
                                     'LUZ VERDE': 'green',
                                     'LUZ AMARILLA': 'orange',
                ...
                st.download_button("Descargar CSV Batch", "batch_validation.csv", "text/csv")
    ...
    with tab3:
        ...
        ejemplo_materiales = [
            {"Material": "LiFePO4", "Tipo": "Catodo batería", "Score": 85, "Status": "✅ Viable"},
            {"Material": "YBa2Cu3O7", "Tipo": "Superconductor", "Score": 45, "Status": "⚠️ Difícil"},
            {"Material": "TiO2", "Tipo": "Fotocatalizador", "Score": 92, "Status": "✅ Viable"},
            {"Material": "Si3N4", "Tipo": "Cerámica técnica", "Score": 78, "Status": "✅ Viable"},
            {"Material": "WC", "Tipo": "Carburo duro", "Score": 35, "Status": "❌ No viable económicamente"},
        ]
...
if __name__ == "__main__":
    main()
                ...
                return {'stable': None, 'energy': None, 'message': 'No data found'}
    except Exception as e:
        return {'stable': None, 'energy': None, 'message': str(e)}

def mechanical_properties(formula):
    # Placeholder for ML predictions
    # Use MEGNet or ALIGNN
    # For now, dummy values
    return {'bulk_modulus': 100, 'shear_modulus': 50, 'young_modulus': 150}

def economic_analysis(formula):
    # Placeholder for cost analysis
    # Simple cost estimation
    comp = Composition(formula)
    # Assume costs per element
    costs = {'Li': 10, 'Fe': 0.1, 'P': 1, 'O': 0.01}  # dummy costs
    total_cost = sum(costs.get(str(el), 1) * amt for el, amt in comp.items())
    return {'cost_per_atom': total_cost / comp.num_atoms}

def synthesizability_scoring(results):
    # Semáforo system
    score = 0
    if results['chemical']['valid']:
        score += 1
    if results['thermo']['stable']:
        score += 1
    if results['mechanical']['bulk_modulus'] > 50:
        score += 1
    if results['economic']['cost_per_atom'] < 5:
        score += 1
    if score >= 3:
        return 'green'
    elif score >= 2:
...
with tab1:
    formula = st.text_input('Enter material formula (e.g., LiFePO4)')
    if st.button('Validate'):
        if formula:
            st.write('Validation in progress...')
            chem = chemical_screening(formula)
            thermo = thermodynamic_stability(formula, mp_api_key)
            mech = mechanical_properties(formula)
            econ = economic_analysis(formula)
            results = {'chemical': chem, 'thermo': thermo, 'mechanical': mech, 'economic': econ}
            semaforo = synthesizability_scoring(results)
            st.write('Results:')
            st.json(results)
            if semaforo == 'green':
                st.success('Green light: High potential')
            elif semaforo == 'yellow':
                st.warning('Yellow light: Moderate potential')
            else:
                st.error('Red light: Low potential')
            # Visualization placeholder
            fig = go.Figure(data=[go.Bar(x=['Chemical', 'Thermo', 'Mechanical', 'Economic'], y=[1 if chem['valid'] else 0, 1 if thermo['stable'] else 0, 1 if mech['bulk_modulus'] > 50 else 0, 1 if econ['cost_per_atom'] < 5 else 0])])
            st.plotly_chart(fig)
        else:
...
with tab2:
    ...
    uploaded_file = st.file_uploader('Upload CSV or JSON of formulas')
    if uploaded_file:
        # Process batch
        st.write('Batch processing placeholder - implement logic here')

with tab3:
    st.header('Agent Chat')
    if openai_api_key:
        client = OpenAI(api_key=openai_api_key)
        prompt = st.text_input('Ask the agent')
        if st.button('Send'):
        ...
        st.write('API key required for agent chat')
